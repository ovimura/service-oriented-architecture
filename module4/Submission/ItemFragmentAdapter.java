...
public class ItemFragmentAdapter extends ArrayAdapter<Item> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (fragment instanceof BiddedItemsFragment) {
            ...
            // Fetch bids remotely (no local storage)
            Float highest = bid_list_controller.getHighestBid(item_controller.getId());
            String highestBidder = bid_list_controller.getHighestBidder(item_controller.getId());

            bidder_tv.setVisibility(View.VISIBLE);
            highest_bid_tv.setVisibility(View.VISIBLE);

            if (highest == null) {
                bidder_tv.setText("Bidder: None");
                highest_bid_tv.setText("Highest Bid: None");
            } else {
                bidder_tv.setText("Bidder: " + highestBidder);
                highest_bid_tv.setText("Highest Bid: " + highest);
            }
        }
